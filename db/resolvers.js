...
const resolvers = {
  Query: {
    ...
    obtenerTransportista: async () => {
      try {
        const transporte = await Transportista.find({});
        return transporte;
      } catch (error) {
        console.log(error);
      }
    ...
    ReporteEstadoPedido: async (_, { estado }) => {
  ...
  Mutation: {
    ...
    nuevoTransportista: async (_, { input }) => {
      ...
      const existeTransportista = await Transportista.findOne({ email });
      ...
      try {
        ...
        const transporte = new Transportista(input);
        transporte.save(); // guardarlo
        return transporte;
      } catch (error) {
        console.log(error);
      }
    ...
    actualizarTransportista: async (_, { id, input }) => {
      // revisar si el producto existe o no
      let transportista = await Transportista.findById(id);
      ...
      // guardarlo en la base de datos
      transportista = await Transportista.findOneAndUpdate({ _id: id }, input, {
        new: true,
      });
    ...
    actualizarVehiculo: async (_, { id, input }) => {
      ...
      // guardarlo en la base de datos
      vehiculo = await Vehiculo.findOneAndUpdate({ _id: id }, input, {
        new: true,
      });
    ...
    eliminarVehiculo: async (_, { id }) => {
      ...
      await Vehiculo.findOneAndDelete({ _id: id });

      return "Vehiculo Eliminado";
    },
    eliminarTransportista: async (_, { id }) => {
      let transportista = await Transportista.findById(id);
      ...
      await Transportista.findOneAndDelete({ _id: id });
